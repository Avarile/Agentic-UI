# Cybernetics - Agentic Centre Helm Chart
This Helm Chart provides an easy, light weight template to deploy Cybernetics - Agentic Centre on Kubernetes

## Variables
In this Chart, Cybernetics will only work with environment Variables. You can Specify Vars and Secret using an existing Secret (This can be generated by [creating an Env File and converting it to a Kubernetes Secret](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#-em-secret-em-) `--from-env-file`)  
...
end with a trailing `/` because Cybernetics appends `/auth/...` callback paths.
...
This renders `ADMIN_PANEL_URL` for Cybernetics's admin OAuth flow. For OpenID SSO,
also register this Cybernetics callback URL with your identity provider:
...
After every Cybernetics replica is on the v2-capable image and all active
...
`LANGFUSE_FANOUT_COLLECTOR_URL` for the Cybernetics app unless those values are
...
map rendered into Cybernetics and the fanout gateway. Tenant API keys can still be added
...
and removal of Cybernetics-only routing attributes before export.

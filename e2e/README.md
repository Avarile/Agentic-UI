# Cybernetics - Agentic Centre e2e

The mock e2e profile is the safest default for generated tests. It starts the app with `e2e/config/librechat.e2e.yaml`, injects an in-process fake LLM (via `LIBRECHAT_TEST_RUN_HOOK`), creates an authenticated e2e user, and avoids real provider credentials.
...
The default instruments inline JavaScript only because instrumenting Cybernetics's
...
That command builds the app, starts the Cybernetics test server (with an in-process fake LLM) when needed, writes `e2e/storageState.json`, and opens Playwright codegen at `/c/new`. The npm script uses `http://localhost:3333` so it does not collide with a normal dev server on `3080`. Raw recordings are written to `e2e/recordings/` and ignored by git.

For a real local Cybernetics config instead of the mock profile:
